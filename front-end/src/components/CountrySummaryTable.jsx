...
export default function CountrySummaryTable({ rows = [], onExport }) {
  return (
    <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onExport}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-300 text-slate-800 text-sm font-semibold hover:bg-slate-50 transition-colors"
        >
          ...
          <tbody>
            ...
            {rows.map((row) => (
              <tr key={row.country} className="border-b border-slate-200 last:border-b-0">
                ...
                <td className="px-6 py-6">
                  <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-lg font-semibold ${row.statusClass || 'bg-emerald-100 text-emerald-700'}`}>
